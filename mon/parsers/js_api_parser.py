"""JS static analysis for API reconstruction -- THIS IS CLONER'S ORIGINAL
api_analyzer.py LOGIC (JSCompilerCore + JSAnalyzerBridge + resolve_url_path
+ advanced_static_analysis), ported as-is. Only the packaging changed: it's
now called from JavascriptAnalyzer through context instead of directly from
main.py.
"""
...
def extract_exact_js_response_schema(js_content, endpoint_func_context):
    ...
    if "status" not in schema and "status" in exact_keys:
        schema["status"] = "TYPE_STRING"
    if "message" not in schema and "message" in exact_keys:
        schema["message"] = "TYPE_STRING"
    return schema
